...
def setup_pre_commit():
    ...
    if not run_command("pre-commit install --config config/.pre-commit-config.yaml", "Installing pre-commit hooks"):
        return False
    
    # Install commit message hooks
    if not run_command("pre-commit install --hook-type commit-msg --config config/.pre-commit-config.yaml", "Installing commit message hooks"):
        return False
    
    # Install pre-push hooks
    if not run_command("pre-commit install --hook-type pre-push --config config/.pre-commit-config.yaml", "Installing pre-push hooks"):
        return False
...
def run_initial_check():
    ...
    result = subprocess.run("pre-commit run --all-files --config config/.pre-commit-config.yaml", shell=True, capture_output=True, text=True)
...
def check_setup():
    ...
    if not run_command("pre-commit status --config config/.pre-commit-config.yaml", "Checking hook installation status"):
        return False
...
def display_usage_info():
    ...
    print("\n⚡ Manual Commands:")
    print("  pre-commit run --all-files --config config/.pre-commit-config.yaml     # Run all hooks on all files")
    print("  pre-commit run black --config config/.pre-commit-config.yaml          # Run only Black formatter")
    print("  pre-commit run pylint --config config/.pre-commit-config.yaml         # Run only Pylint")
    print("  pre-commit run unit-tests --config config/.pre-commit-config.yaml     # Run only unit tests")
...
def main():
    """Main setup function"""
    parser = argparse.ArgumentParser(description="Setup pre-commit hooks for Financial Data Processor")
    parser.add_argument("--install-only", action="store_true", help="Only install dependencies, skip hook setup")
