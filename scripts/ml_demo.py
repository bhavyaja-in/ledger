    ...
    for i, transaction in enumerate(sample_transactions, 1):
        ...
        if summary['suggestions']['category']:
            ...
            for suggestion in summary['suggestions']['category'][:3]:
                print(f"      • {suggestion['category'].title()} "
...
def demo_configuration():
    ...
    print(f"   Min Pattern Length: {default_config['ml']['feature_extraction']['min_pattern_length']}")
    ...
    custom_config = {
        "ml": {
            ...
            "similarity": {
                "fuzzy_threshold": 0.9,    # Stricter matching
    ...
    default_suggestions = len(MLSuggestionService(None, default_config).suggest_transaction_category(test_transaction))
