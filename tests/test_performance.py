...
class TestSystemPerformance:
    ...
    def test_bulk_database_operations_performance(self, performance_monitor, test_config):
        ...
        debit_transactions = session.query(Transaction).filter(Transaction.transaction_type == 'debit').all()
        recent_transactions = session.query(Transaction).order_by(Transaction.created_at.desc()).limit(100).all()
    ...
    def test_icici_transformation_performance(self, performance_monitor, large_transaction_dataset):
        ...
        transformer = IciciBankTransformer(mock_db_manager, config['processors']['icici_bank'], mock_config_loader)
        
        # Prepare large dataset
        transactions = large_transaction_dataset(1000)
        extracted_data = {
            'transactions': [{'data': t} for t in transactions]
        }
        ...
        with patch('builtins.input', return_value='1'), \
             ...
             patch.object(transformer, '_ask_user_for_category', return_value='other'), \
    ...
    def test_end_to_end_processing_performance(self, performance_monitor, large_transaction_dataset):
    ...
    def test_memory_efficiency_large_datasets(self, performance_monitor, large_transaction_dataset):
        ...
        if len(memory_usage) >= 2:
            ratio_1000_to_100 = memory_usage[2][1] / memory_usage[0][1] if memory_usage[0][1] > 0 else 1
            ratio_2000_to_100 = memory_usage[3][1] / memory_usage[0][1] if memory_usage[0][1] > 0 else 1
            
            # Memory usage should not grow more than 30x for 20x data increase
            assert ratio_2000_to_100 < 30, f"Memory usage scaling issue: {ratio_2000_to_100}x increase for 20x data"
    ...
    def test_performance_benchmark_suite(self, performance_monitor):
        ...
        try:
            ...
            benchmark_results['currency_detection'] = {'duration': duration, 'memory': memory}
            
        except ImportError:
            benchmark_results['currency_detection'] = {'duration': 0, 'memory': 0, 'note': 'Not available'}
        ...
        for operation, metrics in benchmark_results.items():
            if 'note' not in metrics:  # Skip unavailable operations
                assert metrics['duration'] < 5.0, f"{operation} took too long: {metrics['duration']:.3f}s"
                assert metrics['memory'] < 100, f"{operation} used too much memory: {metrics['memory']:.1f}MB"
    ...
    def test_single_transaction_performance(self, performance_monitor):
        ...
        assert average_duration < self.THRESHOLDS['single_transaction_process'], \
