...
class TestDatabaseCurrency:
    ...
    def test_query_splits_by_currency(self, db_setup):
        ...
        usd_splits = session.query(TransactionSplit).filter(TransactionSplit.currency == 'USD').all()
        eur_splits = session.query(TransactionSplit).filter(TransactionSplit.currency == 'EUR').all()
