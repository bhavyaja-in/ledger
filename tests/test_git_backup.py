...
class TestGitDatabaseBackup:
    ...
    def test_setup_backup_repo_clone_success(self, mock_run, temp_dir, capsys):
        ...
        with patch.object(GitDatabaseBackup, '_load_config', return_value={}):
            backup = GitDatabaseBackup(
                backup_repo_path=str(repo_path),
                repo_url=repo_url
            )
            result = backup.setup_backup_repo()
    ...
    @patch('os.chdir')
    @patch('os.makedirs')
    def test_setup_backup_repo_create_new(self, mock_makedirs, mock_chdir, mock_run, temp_dir, capsys):
        ...
        with patch.object(GitDatabaseBackup, '_load_config', return_value={}), \
            ...
            expected_calls = [
                ...
                call(['git', 'commit', '-m', 'Initial commit: Setup backup repository'], 
                     check=True, capture_output=True)
            ]
    ...
    @patch('os.chdir')
    @patch('os.makedirs')
    def test_setup_backup_repo_git_error(self, mock_makedirs, mock_chdir, mock_run, temp_dir, capsys):
    ...
    def test_preserve_previous_backup_success(self, mock_datetime, mock_move, mock_chdir, mock_run, temp_dir, capsys):
        ...
        with patch.object(GitDatabaseBackup, '_load_config', return_value={}), \
            ...
            # Verify git commands
            expected_calls = [
                call(['git', 'add', 'financial_data_backup_2023-12-01_12-30-45.db'], 
                     check=True, capture_output=True),
                call(['git', 'commit', '-m', 'Archive previous backup as financial_data_backup_2023-12-01_12-30-45.db'], 
                     check=True, capture_output=True)
            ]
            mock_run.assert_has_calls(expected_calls)
    ...
    def test_commit_backup_success(self, mock_datetime, mock_chdir, mock_run, temp_dir, capsys):
        ...
        with patch.object(GitDatabaseBackup, '_load_config', return_value={}):
            ...
            expected_calls = [
                ...
                call(['git', 'commit', '-m', 'Database backup - 2023-12-01 12:30:45'], 
                     check=True, capture_output=True),
    ...
    def test_restore_backup_exception_handling(self, temp_dir, capsys):
        ...
        with patch.object(GitDatabaseBackup, '_load_config', return_value={}), \
             patch.object(GitDatabaseBackup, '_simple_decrypt', side_effect=Exception("Decrypt error")):
    ...
    @patch('os.chdir')
    @patch('os.listdir')
    def test_show_backup_history_success(self, mock_listdir, mock_chdir, mock_run, temp_dir, capsys):
        ...
        # Mock git log output
        mock_run.return_value = Mock(stdout="abc123 Database backup - 2023-12-01\ndef456 Database backup - 2023-11-30\n")
    ...
    @patch('os.chdir')
    @patch('os.listdir')
    def test_show_backup_history_git_error(self, mock_listdir, mock_chdir, mock_run, temp_dir, capsys):
