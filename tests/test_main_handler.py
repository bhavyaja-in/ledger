...
from src.handlers.main_handler import MainHandler, BackupManager, main
...
class TestBackupManager:
    ...
    def test_create_backup_not_available_startup(self, backup_manager):
        ...
        assert result is False
        mock_print.assert_called_once_with("⚠️  Backup system not configured (continuing without backup)")
...
class TestMainHandler:
    ...
    def main_handler(self, mock_config):
        ...
        with patch('src.handlers.main_handler.ConfigLoader') as mock_config_loader_class, \
    ...
    def test_select_file_browse_option(self, main_handler):
        ...
        files = [{'name': 'file1.xlsx', 'path': '/test/file1.xlsx', 'size': 1024, 'modified': datetime.now()}]
        
        with patch('builtins.input', return_value='2'), \
             patch.object(main_handler, '_browse_for_file', return_value='/custom/file.xlsx') as mock_browse:
    ...
    def test_select_file_back_to_processor(self, main_handler):
        ...
        with patch('builtins.input', return_value='3'), \
             patch.object(main_handler, '_select_processor', return_value='new_processor') as mock_select:
    ...
    def test_browse_for_file_invalid_then_quit(self, main_handler):
        ...
        with patch('builtins.input', side_effect=['/invalid/file.xlsx', 'n']), \
             patch('os.path.exists', return_value=False), \
             patch.object(main_handler, '_select_processor', return_value='back_to_menu') as mock_select, \
             patch('builtins.print'):
    ...
    def test_process_file_success(self, main_handler):
        ...
        with patch.object(main_handler, '_get_or_create_institution', return_value=mock_institution), \
             patch.object(main_handler, '_create_processed_file_record', return_value=mock_processed_file), \
             ...
             patch('time.time', side_effect=[100, 105]):  # 5 second processing time
    ...
    def test_process_file_interrupted(self, main_handler):
        ...
        main_handler.mock_db_loader.update_processed_file_status.assert_called_with(1, 'partially_processed')
    ...
    def test_transform_data_success(self, main_handler):
        ...
        with patch('builtins.__import__', return_value=mock_module):
            result = main_handler._transform_data(transformer_name, extracted_data, mock_institution, mock_processed_file)
        
        assert result == expected_result
        mock_transformer_class.assert_called_once_with(
            main_handler.db_manager, 
            main_handler.config, 
            main_handler.config_loader
        )
    ...
    def test_get_or_create_institution(self, main_handler):
        ...
        main_handler.mock_db_loader.get_or_create_institution.assert_called_once_with('Icici Bank', 'bank')
    ...
    def test_create_processed_file_record(self, main_handler):
        ...
        with patch('os.path.basename', return_value='test_file.xlsx'), \
             patch('os.path.getsize', return_value=1024):
            
            main_handler.mock_db_loader.create_processed_file.return_value = mock_processed_file
            
            result = main_handler._create_processed_file_record(institution_id, file_path, processor_type)
...
class TestMainFunction:
    ...
    def test_main_with_all_arguments(self):
        ...
        mock_handler.run.assert_called_once_with(processor_type='icici_bank', file_path='/test/file.xlsx')
        mock_exit.assert_called_once_with(0)
    ...
    def test_main_fatal_exception(self):
        ...
        with patch('sys.argv', test_args), \
             patch('src.handlers.main_handler.MainHandler', side_effect=Exception("Fatal error")), \
             patch('sys.exit') as mock_exit, \
             patch('builtins.print') as mock_print:
