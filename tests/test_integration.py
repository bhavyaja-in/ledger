...
class TestIntegrationSafety:
    ...
    def ensure_test_environment(self):
        ...
        assert os.environ.get('LEDGER_TEST_MODE') == 'true', "CRITICAL: Test mode must be active for integration tests"
        
        # Verify we're not in any production-like directory
        current_dir = os.getcwd()
        production_indicators = ['production', 'prod', 'live', 'main_db']
        for indicator in production_indicators:
            assert indicator not in current_dir.lower(), f"CRITICAL: Integration tests cannot run in production-like directory: {current_dir}"
        ...
        for db_file in prod_db_files:
            if os.path.exists(db_file):
                ...
                assert not os.access(db_file, os.W_OK), f"CRITICAL: Production database {db_file} must not be writable during tests"
...
class TestEndToEndWorkflowRealistic:
    ...
    @pytest.mark.integration
    @pytest.mark.unit
    def test_complete_processing_simulation(self, integration_test_environment, realistic_transaction_files, test_configurations):
    ...
    @pytest.mark.integration
    @pytest.mark.unit
    def test_currency_detection_integration(self, integration_test_environment, test_configurations):
        ...
        test_cases = [
            ...
            {
                'description': 'ATM WITHDRAWAL EUR 100.00',
                'expected_currency': 'EUR'
            },
            {
                'description': 'REGULAR TRANSACTION',
                'expected_currency': 'INR'  # Default
            }
        ]
             ...
             assert detected_currency in ['INR', 'USD', 'EUR', None], f"Invalid currency detected: {detected_currency}"
...
class TestDatabaseIntegrationRealistic:
    ...
    @pytest.mark.integration 
    @pytest.mark.unit
    def test_transaction_splitting_integration(self, integration_test_environment, test_configurations):
        ...
        try:
            ...
            splits = session.query(TransactionSplit).filter_by(
...
class TestConfigurationIntegration:
    ...
    @pytest.mark.integration
    @pytest.mark.unit
    def test_configuration_loading_complete(self, integration_test_environment, test_configurations):
    ...
    def test_test_mode_isolation_complete(self, integration_test_environment):
        ...
        for prod_file in production_files:
            if os.path.exists(prod_file):
                ...
                assert not os.access(prod_file, os.W_OK), f"Production file {prod_file} should not be writable in test mode"
...
class TestErrorHandlingIntegration:
    ...
    def test_database_error_scenarios(self, integration_test_environment):
        ...
        try:
            ...
            assert any(keyword in str(e).lower() for keyword in ['database', 'connection', 'url', 'invalid'])
...
class TestSecurityIntegration:
    ...
    def test_production_data_isolation(self, integration_test_environment):
        ...
        try:
            ...
            for table in tables:
                assert table.startswith(test_prefix) or table in ['sqlite_sequence'], f"Non-test table found: {table}"
...
class TestPerformanceIntegration:
    ...
    def test_large_dataset_simulation(self, integration_test_environment, test_configurations):
        ...
        assert processing_time < 10.0, f"Performance test took too long: {processing_time:.2f} seconds"
        
        print(f"✅ Performance integration complete: {processing_time:.2f} seconds for 100 transactions")
