...
class TestIciciBankExtractor:
    ...
    def test_extract_success(self, extractor, sample_transaction_data, sample_file_info):
        ...
        extractor.excel_extractor.detect_header_row.assert_called_once_with(mock_df, extractor.required_columns)
    ...
    @pytest.mark.unit
    @pytest.mark.extractor
    def test_extract_with_print_output(self, extractor, sample_transaction_data, sample_file_info, capsys):
    ...
    def test_extract_error_propagation(self, extractor):
        ...
        extractor.excel_extractor.read_excel_file = Mock(side_effect=ValueError("Invalid file format"))
        
        with pytest.raises(Exception, match="Error extracting ICICI Bank data: Invalid file format"):
            extractor.extract(file_path)
        
        # Test custom exception propagation
        extractor.excel_extractor.read_excel_file = Mock(side_effect=Exception("Custom error message"))
        
        with pytest.raises(Exception, match="Error extracting ICICI Bank data: Custom error message"):
            extractor.extract(file_path)
