...
class TestIciciBankTransformer:
    ...
    def mock_config(self):
        ...
        return {
            "categories": [{"name": "food"}, {"name": "transport"}, {"name": "bills"}],
    ...
    def test_init_basic(self, mock_db_manager, mock_config):
        ...
        with patch(
            "src.transformers.icici_bank_transformer.DatabaseLoader"
        ) as mock_db_loader_class, patch("signal.signal") as mock_signal:
    ...
    def test_init_with_config_loader(self, mock_db_manager, mock_config, mock_config_loader):
        ...
        with patch("src.transformers.icici_bank_transformer.DatabaseLoader"), patch(
            "signal.signal"
        ):
            transformer = IciciBankTransformer(mock_db_manager, mock_config, mock_config_loader)
    ...
    def test_process_transaction_interactive_no_enum_found_full_flow(self, transformer):
        ...
        with patch.object(
            transformer, "_check_existing_enum_match", return_value=None
        ), patch.object(
            transformer,
    ...
    def test_handle_existing_enum_match_decline_to_skip(self, transformer):
        ...
        with patch("builtins.input", side_effect=["n", "s"]), patch("builtins.print"), patch.object(
            transformer, "_ask_for_reason", return_value="Not applicable"
        ):
            result = transformer._handle_existing_enum_match(existing_enum, "description")
    ...
    def test_handle_existing_enum_match_decline_to_new_flow(self, transformer):
        ...
        existing_enum = {"id": 1, "enum_name": "grocery_payments", "category": "food"}

        with patch("builtins.input", side_effect=["n", "n"]), patch("builtins.print"), patch.object(
            transformer, "_full_interactive_flow", return_value={"action": "process", "enum_id": 2}
        ) as mock_flow:
            result = transformer._handle_existing_enum_match(existing_enum, "description")
    ...
    def test_full_interactive_flow_complete_success(self, transformer):
        ...
        with patch.object(transformer, "_ask_for_pattern_word", return_value="ATM"), patch.object(
            transformer, "_ask_for_enum_name", return_value="atm_withdrawals"
        ), patch.object(transformer, "_handle_enum_and_category", return_value=123), patch.object(
            transformer, "_ask_for_transaction_category", return_value="cash_withdrawal"
        ), patch(
            "builtins.print"
        ):
    ...
    def test_full_interactive_flow_no_pattern_word(self, transformer):
        ...
        with patch.object(transformer, "_ask_for_pattern_word", return_value=None), patch.object(
            transformer, "_ask_for_reason", return_value="No suitable pattern"
    ...
    def test_ask_for_pattern_word_valid_selection(self, transformer):
        ...
        with patch.object(
            transformer, "_get_pattern_suggestions", return_value=["UPI", "GROCERY", "STORE"]
        ), patch("builtins.input", return_value="2"), patch("builtins.print") as mock_print:
    ...
    def test_ask_for_pattern_word_skip_selection(self, transformer):
        ...
        with patch.object(
            transformer, "_get_pattern_suggestions", return_value=["COMPLEX", "TRANSACTION"]
        ), patch("builtins.input", return_value="3"), patch("builtins.print"):
            result = transformer._ask_for_pattern_word(description)
    ...
    def test_ask_for_pattern_word_invalid_then_valid(self, transformer):
        ...
        with patch.object(
            transformer, "_get_pattern_suggestions", return_value=["TEST", "TRANSACTION"]
        ), patch("builtins.input", side_effect=["invalid", "0", "1"]), patch(
            "builtins.print"
        ) as mock_print:
            result = transformer._ask_for_pattern_word(description)
    ...
    def test_ask_for_enum_name_valid_input(self, transformer):
        ...
        with patch("builtins.input", return_value="grocery_payments"), patch(
            "builtins.print"
        ) as mock_print:
    ...
    def test_ask_for_enum_name_empty_then_valid(self, transformer):
        ...
        with patch("builtins.input", side_effect=["", "atm_withdrawals"]), patch(
            "builtins.print"
        ) as mock_print:
            result = transformer._ask_for_enum_name(pattern_word)
    ...
    def test_handle_enum_and_category_success(self, transformer):
        ...
        with patch.object(transformer, "_ask_for_category", return_value="test_category"), patch(
            "builtins.print"
        ):
        ...
        transformer.db_loader.create_enum.assert_called_once_with(
            enum_name=enum_name, patterns=patterns, category="test_category"
        )
    ...
    def test_ask_for_category_new_category(self, transformer):
        ...
        with patch("builtins.input", side_effect=["3", "new_category"]), patch(
            "builtins.print"
        ) as mock_print:
            result = transformer._ask_for_category()
    ...
    def test_ask_for_category_invalid_then_valid(self, transformer):
        ...
        with patch("builtins.input", side_effect=["invalid", "0", "1"]), patch(
            "builtins.print"
        ) as mock_print:
            result = transformer._ask_for_category()
    ...
    def test_ask_for_transaction_category_existing_selection(self, transformer):
        ...
        with patch.object(
            ...
            return_value={"type": "existing", "value": "groceries"},
        ):
    ...
    def test_ask_for_transaction_category_new_creation(self, transformer):
        ...
        with patch.object(
            transformer,
            "_ask_for_transaction_category_with_options",
            return_value={"type": "new", "value": "organic_food"},
        ):
            result = transformer._ask_for_transaction_category(enum_category)

        assert result == "organic_food"
        transformer.config_loader.add_category.assert_called_once_with(
            "organic_food", parent="food"
        )
    ...
    def test_ask_for_transaction_category_with_options_new(self, transformer):
        ...
        with patch("builtins.input", side_effect=["3", "airplane"]), patch("builtins.print"):
            result = transformer._ask_for_transaction_category_with_options(enum_category)
    ...
    def test_ask_for_reason_valid_input(self, transformer):
        ...
        with patch("builtins.input", return_value="Personal expense"), patch("builtins.print"):
    ...
    def test_ask_for_reason_empty_then_valid(self, transformer):
        ...
        with patch("builtins.input", side_effect=["", "Valid reason"]), patch(
            "builtins.print"
        ) as mock_print:
            result = transformer._ask_for_reason()
    ...
    def test_process_transactions_complete_workflow_success(self, transformer):
        ...
        with patch.object(
            ...
            transformer, "_create_transaction_hash", side_effect=["hash1", "hash2"]
        ), patch.object(
            transformer.db_loader, "check_transaction_exists", return_value=False
        ), patch.object(
            transformer.db_loader, "check_skipped_transaction_exists", return_value=False
        ), patch.object(
            transformer, "_display_transaction"
        ), patch.object(
            transformer,
            ...
            transformer.db_loader, "create_transaction"
        ), patch(
            "builtins.print"
        ):
    ...
    def test_process_transactions_with_duplicates_and_skips(self, transformer):
        ...
        with patch.object(
            ...
            transformer, "_create_transaction_hash", side_effect=["dup_hash", "skip_hash"]
        ), patch.object(
            transformer.db_loader, "check_transaction_exists", side_effect=[True, False]
        ), patch.object(
            transformer, "_display_transaction"
        ), patch.object(
            transformer,
            "_process_transaction_interactive",
            return_value={"action": "skip", "reason": "User skipped"},
        ), patch.object(
    ...
    def test_process_transactions_with_auto_skipped(self, transformer):
        ...
        with patch.object(
            transformer,
            "_transform_transaction",
            return_value={"description": "PREVIOUSLY SKIPPED", "date": datetime(2023, 1, 1)},
        ), patch.object(
            transformer, "_create_transaction_hash", return_value="skipped_hash"
        ), patch.object(
            transformer.db_loader, "check_transaction_exists", return_value=False
        ), patch.object(
    ...
    def test_process_transactions_reprocess_skipped_enabled(self, transformer):
        ...
        with patch.object(
            transformer,
            "_transform_transaction",
            return_value={"description": "REPROCESS ME", "date": datetime(2023, 1, 1)},
        ), patch.object(
            transformer, "_create_transaction_hash", return_value="reprocess_hash"
        ), patch.object(
            transformer.db_loader, "check_transaction_exists", return_value=False
        ), patch.object(
            transformer.db_loader, "check_skipped_transaction_exists", return_value=True
    ...
    def test_process_transactions_exception_handling(self, transformer):
            ...
            "transactions": [
                {
                    "data": {
                        "Transaction Date": "01-01-2023",
                        "Transaction Remarks": "EXCEPTION CASE",
                    }
                }
            ]
        }
        ...
        with patch.object(
            transformer, "_transform_transaction", side_effect=Exception("Test error")
        ), patch("builtins.print"):
            result = transformer.process_transactions(
                extracted_data, mock_institution, mock_processed_file
            )
    ...
    def test_process_transactions_interrupted_flow(self, transformer):
        ...
        with patch.object(
            transformer,
            "_transform_transaction",
            return_value={"description": "INTERRUPTED", "date": datetime(2023, 1, 1)},
        ), patch("builtins.print"):
            result = transformer.process_transactions(
                extracted_data, mock_institution, mock_processed_file
            )
    ...
    def test_handle_skipped_transaction_with_hash(self, transformer):
        ...
        transformer._handle_skipped_transaction(
            row_data, institution_id, processed_file_id, skip_reason, row_number, transaction_hash
        )
