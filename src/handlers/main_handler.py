...
class MainHandler:
        ...
        try:
            ...
            if not processor_type:
                ...
                if processor_type not in self.config['processors']:
                    ...
                    return {'status': 'error', 'message': f'Unknown processor: {processor_type}'}
    ...
    def _process_file(self, processor_type: str, file_path: str) -> Dict[str, Any]:
        ...
        processed_file = self._create_processed_file_record(institution.id, file_path, processor_type)
        ...
        try:
            ...
            result = self._transform_data(transformer_name, extracted_data, institution, processed_file)
            ...
            if result.get('status') == 'interrupted':
                ...
                self.db_loader.update_processed_file_status(processed_file.id, 'partially_processed')
    ...
    def _transform_data(self, transformer_name: str, extracted_data: Dict[str, Any], 
    ...
    def _create_processed_file_record(self, institution_id: int, file_path: str, processor_type: str):
    ...
    def _create_processing_log(self, processed_file_id: int, result: Dict[str, Any], processing_time: float):
    ...
    def _display_summary(self, result: Dict[str, Any]):
        ...
        processed_or_skipped = result.get('processed_transactions', 0) + result.get('skipped_transactions', 0)
        if total_found > 0 and final_status == "PARTIALLY_PROCESSED":
            completion_pct = (processed_or_skipped / total_found) * 100
            print(f"📈 Completion Rate: {completion_pct:.1f}% ({processed_or_skipped}/{total_found})")
